...
from lib.models.gptrack import build_gptrack
...
class GPTrack(BaseTracker):
    def __init__(self, params, dataset_name):
        super(GPTrack, self).__init__(params)
        network = build_gptrack(params.cfg, training=False)
    ...
    def track(self, image, info: dict = None):
        H, W, _ = image[0].shape
        self.frame_id += 1
        x_patch_arr, resize_factor, x_amask_arr = sample_target(image[1], self.state[1], self.params.search_factor,
                                                                output_sz=self.params.search_size)  # (x1, y1, w, h)
        x_patch_arr_last, resize_factor_last, x_amask_arr_last = sample_target(image[0], self.state[0], self.params.search_factor,
                                                                output_sz=self.params.search_size)  # (x1, y1, w, h)
        ...
        with torch.no_grad():
            ...
            out_dict = self.network.forward(
                ...
                search=[x_dict.tensors[:,:3,:,:], x_dict.tensors[:,3:,:,:], x_dict_last.tensors[:,:3,:,:], x_dict_last.tensors[:,3:,:,:]], ce_template_mask=self.box_mask_z)
        ...
        self.state[1] = clip_box(self.map_box_back(pred_box, resize_factor), H, W, margin=10)
        # for debug
        if self.debug:
            if not self.use_visdom:
