...
class Tracker:
    ...
    def run_sequence(self, seq, debug=None, run_id=None, name=None):
        ...
        params.checkpoint = os.path.join(env_settings().save_dir, "%s/checkpoints/train/gptrack/%s/GPTrack_ep%04d.pth.tar" %
                                     (name, name, run_id))
        ...
        for frame_num, frame_path in enumerate(zip(seq.frames[0][1:], seq.frames[1][1:]), start=1):
            ...
            if len(self.confidence) < 16 and len(self.out_box) < 16:
                ...
                if self.confidence[-1] < 0.25:
                    ...
                    for i in range(16):
                        if i == 0:
                            kalman_filter_params = kalman.kf_init(self.out_box[i][0], self.out_box[i][1], 0, 0)  # Initialize.
                        else:
                            kalman_filter_params.z = np.transpose([self.out_box[i][0], self.out_box[i][1]])  # Set the current observed position.
                            kalman_filter_params = kalman.kf_update(kalman_filter_params)  # Kalman filtering.
